...
function Dashboard({ contracts, clients, goTo }) {
  ...
  const collected = contracts.reduce((s,c)=>{
    ...
    return s + va*((c.deposit50Paid?.5:0)+(c.final50Paid?.5:0));
  },0);
...
function IncomeScreen({ contracts }) {
  ...
  const allCollected = contracts.reduce((s,c)=>s+videoOnly(c)*((c.deposit50Paid?.5:0)+(c.final50Paid?.5:0)),0);
  const pend = contracts.filter(c=>c.status!=="cancelled").reduce((s,c)=>s+videoOnly(c)*((!c.deposit50Paid?.5:0)+(!c.final50Paid?.5:0)),0);
      ...
      {sorted.map(c=>(
        <div key={c.id} style={{background:"var(--surface)",border:`1px solid ${SC[c.status]}33`,borderRadius:13,padding:"12px 14px",marginBottom:9,display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{textAlign:"left"}}>
            ...
            <div style={{fontSize:9,color:"var(--muted)",textAlign:"center"}}>{Math.round(((c.deposit50Paid?.5:0)+(c.final50Paid?.5:0))*100)}% مدفوع</div>
          </div>
        </div>
      ))}
    </div>
